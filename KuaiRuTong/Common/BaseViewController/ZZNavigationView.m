...
{
    CGSize titleSize = [title sizeWithFont:[UIFont systemFontOfSize:12.0f] constrainedToSize:CGSizeMake(navigationBack.frame.size.height,CGFLOAT_MAX) lineBreakMode:NSLineBreakByClipping];
    if (titleSize.width>80) {
        titleSize.width = 80;
    }
    if (rightBtn!=nil) {
        rightBtn.frame =  CGRectMake(navigationBack.frame.size.width-2*titleSize.width, navigationBack.frame.size.height/2-titleSize.height/2, titleSize.width*2, titleSize.height);
